...
function generateGrid()
{
    ...
    const x = Math.floor(Math.random() * choiceANames.length);
...
function updateSwarm()
{
    ...
    const droneRatio = Math.max(harvesterLevel + 1, wireDroneLevel + 1) / Math.min(harvesterLevel + 1, wireDroneLevel + 1);
    ...
    const d = Math.floor(harvesterLevel + wireDroneLevel);
...
window.setInterval(function()
{
    ...
    if (investmentEngineFlag===1)
    {
        ...
        {
            const r = formatWithCommas(ledger + portTotal);
...
function load1()
{
    const loadGame = JSON.parse(localStorage.getItem("saveGame1"));
    const loadProjectsUses = JSON.parse(localStorage.getItem("saveProjectsUses1"));
    const loadProjectsFlags = JSON.parse(localStorage.getItem("saveProjectsFlags1"));
    const loadProjectsActive = JSON.parse(localStorage.getItem("saveProjectsActive1"));
    const loadStratsActive = JSON.parse(localStorage.getItem("saveStratsActive1"));
    for(var i=0; i < projects.length; i++)
    {
        projects[i].uses = loadProjectsUses[i];
        projects[i].flag = loadProjectsFlags[i];
